...
// Precio sugerido = costo / 0.7 (deja 30% de margen sobre venta)
...
// Mismo calculo que usa el backend para consistencia.
...
export default function Precios() {
  ...
  const agregarFruta = async (e) => {
    ...
    setAgregando(true);
    try {
      const { data: creada } = await api.post('/productos', { nombre: nuevaFruta.trim() });
      ...
      // 409 = producto ya existe pero activo. Si tenemos su ID y hay costo, guardamos el precio.
  ...
  const quitarFruta = async (productoId, nombre) => {
    ...
    catch (err) { setMsg(err.response?.data?.error || 'No se pudo quitar'); }
  };
  ...
  const quitarPrecioHoy = async (productoId, nombre) => {
    ...
    catch (err) { setMsg(err.response?.data?.error || 'No se pudo quitar el precio'); }
  };
